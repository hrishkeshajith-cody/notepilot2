...
import { HelpCircle, ChevronDown, ChevronUp, Award, Download } from "lucide-react";
...
const downloadQuestions = (questions: ImportantQuestions) => {
  let content = `IMPORTANT QUESTIONS\n${'='.repeat(50)}\n\n`;
  
  if (questions.one_mark && questions.one_mark.length > 0) {
    content += `1-MARK QUESTIONS\n${'-'.repeat(20)}\n`;
    questions.one_mark.forEach((q, idx) => {
      content += `${idx + 1}. ${q}\n`;
    });
    content += '\n';
  }
  
  if (questions.three_mark && questions.three_mark.length > 0) {
    content += `3-MARK QUESTIONS\n${'-'.repeat(20)}\n`;
    questions.three_mark.forEach((q, idx) => {
      content += `${idx + 1}. ${q}\n`;
    });
    content += '\n';
  }
  
  if (questions.five_mark && questions.five_mark.length > 0) {
    content += `5-MARK QUESTIONS\n${'-'.repeat(20)}\n`;
    questions.five_mark.forEach((q, idx) => {
      content += `${idx + 1}. ${q}\n`;
    });
    content += '\n';
  }
  ...
  const totalQuestions = 
    (questions.one_mark?.length || 0) + 
    (questions.three_mark?.length || 0) + 
    (questions.five_mark?.length || 0);
  ...
  link.href = url;
  link.download = `important-questions-${Date.now()}.txt`;
